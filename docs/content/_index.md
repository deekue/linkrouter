...
- See [PRIVACY.md](/PRIVACY) for the full privacy policy.
...
{{/* Read build.gradle.kts from two levels up relative to docs/ */}}
{{ $gradle := readFile "../app/build.gradle.kts" }}

{{/* Extract content inside quotes after versionName */}}
{{ $version := replaceRE `(?m).*versionName\s*=\s*["']([^"']+)["'].*` "$1" $gradle }}
